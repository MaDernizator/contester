...
def assign_problem_insert_position(
    *,
    contest_id,
    requested_position: int | None,
) -> int:
    ...
    return _rebalance_for_insert(
        siblings,
        requested_position=requested_position,
    )


def move_problem_to_position(
    *,
    problem: Problem,
    requested_position: int | None,
) -> int:
    siblings = db.session.execute(
        select(Problem)
        .where(Problem.contest_id == problem.contest_id)
        .order_by(Problem.position.asc(), Problem.created_at.asc(), Problem.id.asc())
    ).scalars().all()
...
def assign_test_case_insert_position(
    *,
    problem_id,
    requested_position: int | None,
) -> int:
...
def move_test_case_to_position(
    *,
    test_case: TestCase,
    requested_position: int | None,
) -> int:
    siblings = db.session.execute(
        select(TestCase)
        .where(TestCase.problem_id == test_case.problem_id)
        .order_by(TestCase.position.asc(), TestCase.created_at.asc(), TestCase.id.asc())
    ).scalars().all()
